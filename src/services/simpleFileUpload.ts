...
export const uploadPatientFile = async (
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Cannot connect to backend server. Please make sure the backend server is running and the proxy is configured correctly. Error: ${healthError.message}`);
    }
